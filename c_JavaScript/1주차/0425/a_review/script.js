...
let isRaining = false; 
let ans = isRaining ?  `비가 오고 있습니다` : `비가 오지 않습니다`;
console.log(`현재 ${city}의 기온은 ${temperature}도이며, ${isRaining}`);
...
console.log(`계산식의 결과는 ${magic}입니다.`);
...
let originalPrice = Number(prompt(`원가`)); // 원가 입력받음
let discountRate = Number(prompt('할인율')); // 할인율 입력받음
let discountAmount = discountRate / 100 * originalPrice; // 할인 금액 계산
...
let score = Number(prompt(`점수는 ?`));
let hasTrophy = (prompt('트로피를 가지고 있습니까?')); // 사용자로부터 트로피를 가지고 있는지 답변을 받아 변수에 저장 (yes/no의 데이터 입력받기 - yes와 동일할 경우 해당 변수에 true값 저장)
let gameWins = Number(prompt(`우승 횟수`));

let isEligible = score >= 5000 ? (hasTrophy = 'yes' || hasTrophy >= 3 ? `예` : `아니요`) : `아니요`;  // 위의 모든 조건을 만족할 경우 해당 변수에 true값 저장
